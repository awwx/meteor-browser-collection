...
(function() {
  var collections, db, each_sql_result, idOf, result_as_array, _when,
  ...
  _.extend(Meteor.BrowserSQLCollection.prototype, {
    ...
    _reload_all: function() {
      ...
      return db.transaction((function(tx) {
        ...
        var cache_ids, doc, doc_ids, id, toDelete, _i, _j, _len, _len1, _ref, _results;
        doc_ids = _.map(docs, idOf);
        cache_ids = _.map(_this._localCollection.find().fetch(), idOf);
        toDelete = _.without(cache_ids, doc_ids);
        for (_i = 0, _len = toDelete.length; _i < _len; _i++) {
          id = toDelete[_i];
          _this._localCollection.remove(id);
        }
        _results = [];
        for (_j = 0, _len1 = docs.length; _j < _len1; _j++) {
          doc = docs[_j];
          if (_ref = doc._id, __indexOf.call(cache_ids, _ref) >= 0) {
            _results.push(_this._localCollection.update(doc._id, doc));
          } else {
            _results.push(_this._localCollection.insert(doc));
          }
        }
        return _results;
      }));
